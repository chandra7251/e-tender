<?php $__env->startSection('title', 'Sanggahan & Banding'); ?>
<?php $__env->startSection('page-title', 'Manajemen Sanggahan & Banding'); ?>
...
<div class="space-y-6">
    ...
    <div class="flex flex-wrap gap-3">
        <?php $__currentLoopData = [''=> 'Semua','pending'=>'Pending','reviewed'=>'Ditinjau','accepted'=>'Diterima','rejected'=>'Ditolak']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $val => $label): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
        <a href="<?php echo e(request()->fullUrlWithQuery(['status' => $val, 'page' => null])); ?>"
           class="rounded-full px-4 py-1.5 text-xs font-semibold border transition
                   <?php echo e(request('status') === $val ? 'bg-[#3553A8] text-white border-[#3553A8]' : 'bg-white text-gray-600 border-gray-300 hover:border-[#3553A8] hover:text-[#3553A8]'); ?>">
             <?php echo e($label); ?>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <table class="w-full text-sm">
            ...
            <tr class="border-t border-gray-100 hover:bg-gray-50">
                ...
                <td class="px-4 py-3 font-medium text-gray-800 max-w-[180px] truncate" title="<?php echo e($c->tender->title ?? ''); ?>"><?php echo e($c->tender->title ?? '-'); ?></td>
                <td class="px-4 py-3 text-gray-700"><?php echo e(optional($c->vendor->user)->name ?? '-'); ?></td>
                <td class="px-4 py-3">
                    <span class="rounded-full px-2 py-0.5 text-xs font-semibold <?php echo e($typeColor); ?>"><?php echo e(strtoupper($c->type)); ?></span>
                </td>
                <td class="px-4 py-3">
                    <span class="rounded-full px-2 py-0.5 text-xs font-semibold <?php echo e($statusColor); ?>"><?php echo e(strtoupper($c->status)); ?></span>
                </td>
                <td class="px-4 py-3 text-xs <?php echo e($isExpired ? 'text-red-500 font-semibold' : 'text-gray-500'); ?>">
                    <?php echo e($c->deadline ? $c->deadline->format('d/m/Y H:i') : '-'); ?>

                    <?php if($isExpired): ?> <span class="text-red-400">(Expired)</span> <?php endif; ?>
                </td>
                <td class="px-4 py-3 text-gray-500 text-xs"><?php echo e($c->created_at->format('d/m/Y')); ?></td>
                <td class="px-4 py-3">
                    <?php if($c->status === 'pending'): ?>
                    <button onclick="openModal(<?php echo e($c->id); ?>, '<?php echo e(addslashes($c->reason)); ?>')"
                            class="rounded bg-[#3553A8] px-3 py-1.5 text-xs font-semibold text-white hover:bg-[#2B438A] transition">
                        Tanggapi
                    </button>
                    <?php else: ?>
                    <span class="text-xs text-gray-500 italic"><?php echo e(Str::limit($c->response ?? '-', 50)); ?></span>
                    <?php endif; ?>
                </td>
            </tr>
        ...
        <div class="px-4 py-3 border-t border-gray-100"><?php echo e($complaints->links()); ?></div>
    </div>
</div>
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\project-laravel\lelang-2.0\resources\views/admin/complaints/index.blade.php ENDPATH**/ ?>