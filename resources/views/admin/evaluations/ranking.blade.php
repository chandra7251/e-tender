...
@section('title', 'Ranking Evaluasi')
@section('page-title', 'Ranking Evaluasi Tender')
...
<div class="w-full space-y-6">
        ...
        <div class="p-5 flex flex-wrap gap-3">
            @foreach ($criteria as $c)
                <div class="rounded-lg border border-gray-200 px-4 py-3 min-w-[150px]">
                    ...
                    <p class="text-lg font-bold text-[#3553A8] mt-0.5">{{ number_format($c->weight, 0) }}%</p>
                    <p class="text-[10px] text-gray-400">Maks skor: {{ $c->max_score }}</p>
                </div>
            @endforeach
        </div>
    </div>
    ...
    <div class="rounded-xl bg-white shadow-sm border border-gray-200 overflow-hidden">
        <div class="border-b border-gray-200 px-5 py-3 bg-gray-50 flex items-center justify-between">
            ...
            @if ($tender->status === 'closed' && !$tender->hasWinner())
            ...
            <table class="w-full text-sm" style="min-width: 800px;">
                <thead>
                    <tr class="border-b border-gray-200 text-left text-xs font-bold uppercase tracking-widest text-gray-500 bg-gray-50">
                        ...
                        @foreach ($criteria as $c)
                            <th class="px-5 py-3 text-center">{{ $c->name }}<br><span class="font-normal">({{ number_format($c->weight, 0) }}%)</span></th>
                        @endforeach
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors {{ $i === 0 ? 'bg-emerald-50' : '' }}">
                        <td class="px-5 py-3 whitespace-nowrap">
                            @if ($i === 0)
                                <span class="rounded-full bg-[#28C5D4] px-2.5 py-0.5 text-xs font-bold text-white">★ #1</span>
                            @else
                                <span class="text-gray-500 font-semibold">#{{ $i + 1 }}</span>
                            @endif
                        </td>
                        <td class="px-5 py-3 font-semibold text-gray-800 whitespace-nowrap">
                            {{ $bid->vendor->company_name ?? '-' }}
                        </td>
                        <td class="px-5 py-3 text-right font-mono font-bold whitespace-nowrap {{ $i === 0 ? 'text-[#28C5D4]' : 'text-gray-700' }}">
                            Rp {{ number_format($bid->bid_amount, 0, ',', '.') }}
                        </td>
                        @foreach ($bid->evaluation_details as $detail)
                        <td class="px-5 py-3 text-center">
                            <span class="font-mono font-bold text-gray-800">{{ number_format($detail['raw_score'], 1) }}</span>
                            <span class="text-gray-400 text-xs">/{{ $detail['max_score'] }}</span>
                            <br>
                            <span class="text-xs text-[#3553A8] font-semibold">{{ number_format($detail['weighted_score'], 2) }}</span>
                        </td>
                        @endforeach
                        <td class="px-5 py-3 text-center">
                            @if ($bid->total_weighted_score !== null)
                                <span class="text-lg font-bold {{ $i === 0 ? 'text-[#28C5D4]' : 'text-gray-800' }}">
