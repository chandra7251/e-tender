...
@section('page-title', 'Audit Log & Compliance')
...
<div class="w-full space-y-6">
    ...
    <div class="flex items-center justify-between">
        <p class="text-sm text-gray-500">Total: <strong>{{ $logs->total() }}</strong> log</p>
        <a href="{{ route('admin.reports.export.audit-logs', request()->query()) }}"
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                        <td class="px-5 py-3 whitespace-nowrap">
                            <span class="font-mono text-xs text-gray-600">{{ $log->performed_at->format('d/m/Y H:i:s') }}</span>
                        </td>
                        ...
                        <td class="px-5 py-3 whitespace-nowrap">
                            <span class="rounded-full px-2.5 py-0.5 text-xs font-bold
                                @switch($log->module)
                                    ...
                                    @case('vendor') bg-purple-100 text-purple-700 @break
                                    @case('tender') bg-indigo-100 text-indigo-700 @break
                                    @case('evaluation') bg-teal-100 text-teal-700 @break
                                    @case('report') bg-amber-100 text-amber-700 @break
                                    @default bg-gray-100 text-gray-700
                                @endswitch
                            ">{{ ucfirst($log->module) }}</span>
                        </td>
