...
Route::middleware('auth:api')->group(function () {
    ...
    Route::delete('notifications/all', [NotificationController::class, 'destroyAll'])->name('api.notifications.destroyAll');
    Route::patch('notifications/read-all', [NotificationController::class, 'markAllAsRead'])->name('api.notifications.readAll');
...
Route::middleware(['auth:api', 'role:admin,super_admin'])->prefix('admin')->name('api.admin.')->group(function () {
    ...
    Route::get('tenders/{tender}/ranking', [EvaluationApiController::class, 'ranking'])->name('tenders.ranking');
    Route::get('dashboard/stats', [DashboardApiController::class, 'stats'])->name('dashboard.stats');
});

// ─── Public Settings (White-Label) ───────────────────────────────────────────
Route::get('settings/public', [InstansiSettingController::class, 'publicSettings'])->name('api.settings.public');

// ─── Tender Items / BQ (public read) ─────────────────────────────────────────
Route::get('tenders/{tender}/items', [TenderItemController::class, 'index'])->name('api.tenders.items.index');

Route::middleware('auth:api')->group(function () {

    // ─── Vendor: Sanggahan & Banding ──────────────────────────────────────────
    Route::get('complaints', [TenderComplaintController::class, 'index'])->name('api.complaints.index');
    Route::post('tenders/{tender}/complaints', [TenderComplaintController::class, 'store'])->name('api.tenders.complaints.store');

    // ─── Vendor: Kualifikasi ──────────────────────────────────────────────────
    Route::get('vendors/qualification', [VendorQualificationController::class, 'show'])->name('api.vendors.qualification');
    Route::put('vendors/qualification', [VendorQualificationController::class, 'updateQualification'])->name('api.vendors.qualification.update');
    Route::post('vendors/certifications', [VendorQualificationController::class, 'storeCertification'])->name('api.vendors.certifications.store');
    Route::delete('vendors/certifications/{id}', [VendorQualificationController::class, 'destroyCertification'])->name('api.vendors.certifications.destroy');

    // ─── Vendor: Kontrak ──────────────────────────────────────────────────────
    Route::get('vendor/contracts', [ContractController::class, 'vendorContracts'])->name('api.vendor.contracts.index');
    Route::get('contracts/{contract}', [ContractController::class, 'show'])->name('api.contracts.show');
    Route::patch('contracts/{contract}/sign-vendor', [ContractController::class, 'vendorSign'])->name('api.contracts.sign-vendor');

    // ─── Vendor: Contract Delivery ────────────────────────────────────────────
    Route::patch('contracts/{contract}/deliveries/{delivery}/submit',
        [ContractDeliveryController::class, 'vendorUpdate'])->name('api.contracts.deliveries.submit');
});

Route::middleware(['auth:api', 'role:admin,super_admin'])->prefix('admin')->name('api.admin.')->group(function () {

    // ─── Admin: Instansi Settings ─────────────────────────────────────────────
    Route::get('settings', [InstansiSettingController::class, 'index'])->name('settings.index');
    Route::put('settings', [InstansiSettingController::class, 'update'])->name('settings.update');
    Route::post('settings/logo', [InstansiSettingController::class, 'uploadLogo'])->name('settings.logo');

    // ─── Admin: Sanggahan ─────────────────────────────────────────────────────
    Route::get('complaints', [TenderComplaintController::class, 'adminIndex'])->name('complaints.index');
    Route::patch('complaints/{complaint}/respond', [TenderComplaintController::class, 'respond'])->name('complaints.respond');

    // ─── Admin: Tender Items / BQ ─────────────────────────────────────────────
    Route::post('tenders/{tender}/items', [TenderItemController::class, 'sync'])->name('tenders.items.sync');
    Route::delete('tenders/{tender}/items/{item}', [TenderItemController::class, 'destroy'])->name('tenders.items.destroy');

    // ─── Admin: Kontrak Digital ───────────────────────────────────────────────
    Route::get('contracts', [ContractController::class, 'index'])->name('contracts.index');
    Route::post('contracts', [ContractController::class, 'store'])->name('contracts.store');
    Route::patch('contracts/{contract}/send', [ContractController::class, 'sendToVendor'])->name('contracts.send');
    Route::patch('contracts/{contract}/sign-admin', [ContractController::class, 'adminSign'])->name('contracts.sign-admin');
    ...
    Route::patch('contracts/{contract}/deliveries/{delivery}/verify',
        [ContractDeliveryController::class, 'adminVerify'])->name('contracts.deliveries.verify');
    ...
    // ─── Admin: Export PDF ────────────────────────────────────────────────────
    Route::get('tenders/{tender}/export/rekap', [ExportPdfController::class, 'rekapTender'])->name('tenders.export.rekap');
    Route::get('tenders/{tender}/export/ba-evaluasi', [ExportPdfController::class, 'beritaAcaraEvaluasi'])->name('tenders.export.ba-evaluasi');
    Route::get('tenders/{tender}/export/ba-pemenang', [ExportPdfController::class, 'beritaAcaraPemenang'])->name('tenders.export.ba-pemenang');
    Route::get('contracts/{contract}/export/pdf', [ExportPdfController::class, 'contractPdf'])->name('contracts.export.pdf');
});

// ─── E-Catalogue (Public) ─────────────────────────────────────────────────────
Route::get('catalogue', [VendorCatalogueController::class, 'index']);
Route::get('catalogue/categories', [VendorCatalogueController::class, 'categories']);
Route::get('catalogue/{id}', [VendorCatalogueController::class, 'show'])->where('id', '[0-9]+');

// ─── E-Catalogue (Vendor Auth) ────────────────────────────────────────────────
Route::middleware('auth:api')->prefix('vendor/catalogue')->name('api.vendor.catalogue.')->group(function () {
    Route::get('/', [VendorCatalogueController::class, 'myItems'])->name('index');
    Route::post('/', [VendorCatalogueController::class, 'store'])->name('store');
    Route::put('/{id}', [VendorCatalogueController::class, 'update'])->name('update');
    Route::delete('/{id}', [VendorCatalogueController::class, 'destroy'])->name('destroy');
});

// ─── AI / Machine Learning ─────────────────────────────────────────────────
Route::prefix('ai')->middleware('auth:api')->group(function () {
    Route::post('predict-price', [AiController::class, 'predictPrice']);
    Route::post('detect-anomaly', [AiController::class, 'detectAnomaly']);
    Route::get('score-vendors/{tenderId}', [AiController::class, 'scoreVendors']);
    Route::get('analyze/{tenderId}', [AiController::class, 'analyzeTender']);
});

// ─── Blockchain Verification ───────────────────────────────────────────────
Route::get('blockchain/verify', [BlockchainController::class, 'publicVerify']);
Route::middleware('auth:api')->prefix('blockchain')->group(function () {
    Route::post('record', [BlockchainController::class, 'record']);
    Route::get('verify/{id}', [BlockchainController::class, 'verify']);
    Route::get('chain/{tenderId}', [BlockchainController::class, 'tenderChain']);
});

// ─── Payment Gateway ───────────────────────────────────────────────────────
Route::post('payment/notification', [PaymentController::class, 'notification']);
Route::get('payment/client-key', [PaymentController::class, 'clientKey']);
Route::middleware('auth:api')->prefix('payment')->group(function () {
    Route::post('deposit', [PaymentController::class, 'createDeposit']);
    Route::get('tender/{tenderId}', [PaymentController::class, 'tenderPayments']);
    Route::post('refund/{paymentId}', [PaymentController::class, 'refundDeposit']);
});
