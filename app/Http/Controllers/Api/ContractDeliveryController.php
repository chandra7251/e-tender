...
class ContractDeliveryController extends Controller
{
    ...
    public function vendorUpdate(Request $req, Contract $contract, ContractDelivery $delivery): JsonResponse
    {
        ...
        $delivery = $contract->deliveries()->find($delivery->id);
        if (! $delivery) {
            return $this->error('Delivery tidak ditemukan.', null, 404);
        }
        if (! in_array($delivery->status, ['scheduled', 'in_progress'])) {
            return $this->error('Milestone sudah selesai.', null, 422);
        }
        $req->validate([
            'vendor_notes' => 'nullable|string',
            'evidence' => ['nullable', 'file', 'mimes:jpg,jpeg,png', 'mimetypes:image/jpeg,image/png', 'max:10240'],
        ]);
        $data = ['status' => 'in_progress', 'vendor_notes' => $req->vendor_notes];
        if ($req->hasFile('evidence')) {
            $path = $req->file('evidence')->store("contracts/{$contract->id}/evidence", 'local');
    ...
    public function adminVerify(Contract $contract, ContractDelivery $delivery): JsonResponse
    {
        ...
        if ($delivery->status !== 'delivered') {
            return $this->error('Milestone belum diserahkan vendor.', null, 422);
        }
        $delivery->update(['status' => 'verified', 'verified_at' => now(), 'verified_by' => auth('api')->id()]);
