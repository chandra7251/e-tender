...
class TenderComplaintController extends Controller
{
    ...
    public function store(Request $req, Tender $tender): JsonResponse
    {
        ...
        if (! $tender->result) {
            return $this->error('Hasil tender belum diumumkan.', null, 422);
        }
        $windowDays = (int) InstansiSetting::get('sanggahan_window_days', 5);
        $deadline = $tender->result->created_at->addWeekdays($windowDays);
        if (now()->gt($deadline)) {
            return $this->error("Batas waktu sanggahan ({$windowDays} hari kerja) sudah berakhir.", null, 422);
        }
        // Cek vendor ikut tender
        $participated = $tender->participants()->where('vendor_id', $vendor->id)->exists();
        if (! $participated) {
            return $this->error('Anda tidak terdaftar sebagai peserta tender ini.', null, 403);
        }
        ...
        if ($existing) {
            return $this->error('Anda sudah memiliki sanggahan yang sedang diproses.', null, 422);
        }
    ...
    public function respond(Request $req, TenderComplaint $complaint): JsonResponse
    {
        ...
        $complaint->update(['status' => $req->status, 'response' => $req->response, 'responded_by' => auth('api')->id(), 'responded_at' => now()]);
